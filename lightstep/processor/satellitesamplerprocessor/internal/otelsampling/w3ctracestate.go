...
func NewW3CTraceState(input string) (w3c W3CTraceState, _ error) {
	...
	err := w3cSyntax.scanKeyValues(input, func(key, value string) error {
		...
		case otelVendorCode:
			...
			return err
		case cloudObsVendorCode:
			var err error
			w3c.cots, err = NewCloudObsTraceState(value)
			return err
		default:
...
func (w3c *W3CTraceState) HasAnyValue() bool {
	return w3c.OTelValue().HasAnyValue() || w3c.CloudObsValue().HasAnyValue() || len(w3c.ExtraValues()) != 0
}
...
func (w3c *W3CTraceState) CloudObsValue() *CloudObsTraceState {
	return &w3c.cots
}
...
func (w3c *W3CTraceState) Serialize(w io.StringWriter) error {
	...
	if w3c.cots.HasAnyValue() {
		sep()
		ser.write("sn=")
		ser.check(w3c.cots.Serialize(w))
	}
